...
// Basic CSV parse supporting quoted values with commas (simple)
...
function splitCSVLine(line){
  ...
  for(let i=0;i<line.length;i++){
    ...
    if(ch===',' && !inQuotes){ res.push(cur.trim()); cur=''; } else cur+=ch;
  }
  res.push(cur.trim());
  return res;
}
...
function computeMetrics(headers, rows){
  const nameCol=0; const subjectNames=headers.slice(1);
  const students = rows.map((r, idx)=>{
    const name = r[0] || ('Student '+(idx+1));
    const marks = subjectNames.map((_,i)=>{
      const raw = r[i+1]; return isNumeric(raw)?Number(raw):NaN;
    });
...
// Charts (simple) — avoid external libs
function drawBarChart(students){
  const c=avgBar; const ctx=c.getContext('2d'); ctx.clearRect(0,0,c.width,c.height);
  if(!students.length) return;
  const padding=30; const chartW=c.width - padding*2; const chartH=c.height - padding*2;
  ctx.strokeStyle='#e6eef8'; ctx.lineWidth=1; ctx.beginPath(); ctx.moveTo(padding,padding); ctx.lineTo(padding,padding+chartH); ctx.lineTo(padding+chartW,padding+chartH); ctx.stroke();
  const max = Math.max(...students.map(s=>s.avg),100);
  const gap=8; const barW = (chartW - gap*(students.length+1))/students.length;
  students.forEach((s,i)=>{
    const x = padding + gap + i*(barW+gap);
    const h = (s.avg/max)*chartH; const y = padding+chartH - h;
...
function drawSubjectLine(metrics){
  const c=subjectLine; const ctx=c.getContext('2d'); ctx.clearRect(0,0,c.width,c.height);
  const {subjectNames, students} = metrics; if(!students.length) return;
  ...
  // simple polyline
  const padding=30; const w=c.width-padding*2; const h=c.height-padding*2;
...
function drawGradePie(gradeDist){
  const c=gradePie; const ctx=c.getContext('2d'); ctx.clearRect(0,0,c.width,c.height);
  ...
  keys.forEach(k=>{
    const val = gradeDist[k]; const angle = (val/total)*Math.PI*2;
    ctx.beginPath(); ctx.moveTo(c.width/2,c.height/2);
    ctx.arc(c.width/2,c.height/2,80,start,start+angle); ctx.closePath(); ctx.fillStyle = palette[i % palette.length]; ctx.fill(); start += angle; i++;
  });
  // legend
  ctx.font='12px sans-serif'; let lx=10, ly=10; i=0; keys.forEach(k=>{ ctx.fillStyle=palette[i%palette.length]; ctx.fillRect(lx,ly,12,12); ctx.fillStyle='#000'; ctx.fillText(`${k} (${gradeDist[k]})`, lx+18, ly+11); ly+=18; i++; });
}

// Events
fileInput.addEventListener('change', e=>{ const f=e.target.files[0]; if(!f) return; const r=new FileReader(); r.onload=ev=>{ csvText.value = ev.target.result; }; r.readAsText(f); });

parseBtn.addEventListener('click', ()=>{
  const text = csvText.value.trim(); if(!text) return alert('Paste or upload CSV');
  const {headers, rows} = parseCSV(text);
  if(headers.length<2) return alert('CSV must have at least Name + 1 subject');
  const metrics = computeMetrics(headers, rows); currentData={headers, rows, metrics};
  renderMetrics(metrics);
  localStorage.setItem('spa_last', text);
});

clearBtn.addEventListener('click', ()=>{ csvText.value=''; fileInput.value=''; resultsTable.innerHTML=''; currentData=null; totalStudents.textContent='0'; const ctx=avgBar.getContext('2d'); ctx.clearRect(0,0,avgBar.width,avgBar.height); localStorage.removeItem('spa_last'); });

applyFilterBtn.addEventListener('click', ()=>{
  if(!currentData) return alert('No data');
  const minAvg = Number(minAvgInput.value) || 0; let students = [...currentData.metrics.students];
  students = students.filter(s=>s.avg>=minAvg);
  const sorted = sortBy.value; if(sorted==='avg_desc') students.sort((a,b)=>b.avg-a.avg);
  if(sorted==='avg_asc') students.sort((a,b)=>a.avg-b.avg);
  if(sorted==='name') students.sort((a,b)=>a.name.localeCompare(b.name));
  const filteredMetrics = {...currentData.metrics, students}; renderMetrics(filteredMetrics);
});
...
loadSample.addEventListener('click', ()=>{ fetch('data/sample.csv').then(r=>r.text()).then(t=>{ csvText.value=t; }); });

// load last dataset
window.addEventListener('load', ()=>{ const last = localStorage.getItem('spa_last'); if(last) csvText.value = last; });
